...
import { MessageSquare, Send, User, Building2, CheckCheck, Gift } from 'lucide-react'
...
export default function PartnerMessagesPage() {
    ...
    async function handleSendMessage() {
        ...
        setSending(true)
        const result = await sendMessage(selectedConversation, newMessage.trim(), 'PARTNER')
                ...
                {conversations.length === 0 ? (
                    <div className="flex flex-col items-center justify-center flex-1 px-8 text-center">
                        ...
                        <p className="text-sm text-gray-600">
                            Les startups peuvent t'envoyer des messages et invitations ici.
                        </p>
                    </div>
                ) : (
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 mt-1">
                                            {new Date(convo.last_at).toLocaleDateString('fr-FR')}
                                        </p>
                                    </div>
                                </div>
                            </button>
                ...
                {selectedConvo ? (
                                    ...
                                    >
                                        {msg.is_invitation && (
                                            <div className="flex items-center gap-2 mb-2 pb-2 border-b border-white/30">
                                                ...
                                                <span className="text-sm font-medium">Invitation à une mission</span>
                                            </div>
                                        )}
                                        <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                                        <div className={`flex items-center gap-1 mt-1 ${msg.sender_type === 'PARTNER' || msg.is_invitation ? 'text-white/70' : 'text-gray-400'
                                            }`}>
                                            <span className="text-xs">
                                                {new Date(msg.created_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                                            </span>
                                            {msg.sender_type === 'PARTNER' && msg.read_at && (
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <div className="text-center px-8">
                            ...
                            <p className="text-sm text-gray-600">
                                Choisis une conversation à gauche pour voir les messages.
                            </p>
                        </div>
                    </div>
                )}
