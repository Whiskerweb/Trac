...
function StatPill({ icon: Icon, label, value, color }: {
    ...
    return (
        <motion.div
            ...
            className="flex items-center gap-3 bg-white border border-gray-100/80 rounded-2xl px-5 py-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]"
        >
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-3 gap-3">
                <StatPill icon={Users} label="Total" value={totalMembers} color="bg-violet-50 text-violet-600" />
            ...
            <motion.div
                ...
                className="bg-white rounded-2xl border border-gray-100/80 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-6"
            >
                <div className="flex items-center gap-2.5 mb-5">
                    ...
                    <div>
                        <h2 className="text-[15px] font-semibold text-gray-900">Invite a Seller</h2>
                        <p className="text-xs text-gray-400">Send an email invitation to join your organization</p>
                    </div>
                </div>
                <form onSubmit={handleInvite} className="flex gap-3">
                    <div className="flex-1 relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-gray-50/80 border border-gray-200/60 rounded-xl text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-violet-500/15 focus:border-violet-300/60 focus:bg-white transition-all"
                        />
                    </div>
                    <button
                        type="submit"
                        disabled={inviting || !inviteEmail.trim()}
                        className="px-5 py-3 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800 active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none flex items-center gap-2 transition-all shadow-sm"
                    >
                        {inviting ? (
                        ...
                        <motion.div className="space-y-2" variants={container} initial="hidden" animate="show">
                            {pendingMembers.map((m: any) => (
                                <motion.div
                                    key={m.id}
                                    variants={item}
                                    className="flex items-center justify-between px-4 py-3.5 bg-white border border-gray-100/80 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.03)] hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition-shadow"
                                >
                                        ...
                                        {actionLoading === m.id ? (
                                            ...
                                            <>
                                                ...
                                                <button
                                                    ...
                                                    className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg active:scale-[0.97] transition-all"
                                                    title="Reject"
                                                >
                                                    <X className="w-4 h-4" />
                                                </button>
            ...
            >
                    ...
                    {activeMembers.length > 4 && (
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-8.5 pr-3 py-1.5 text-xs bg-gray-50 border border-gray-100 rounded-lg w-48 focus:outline-none focus:ring-1 focus:ring-violet-500/20 focus:border-violet-300/50 transition-all placeholder:text-gray-300"
                ...
                {activeMembers.length === 0 ? (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="flex flex-col items-center justify-center py-16 bg-white border border-gray-100/80 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)]"
                    >
                        <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center mb-4">
                            <Users className="w-5 h-5 text-gray-300" />
                        </div>
                        <p className="text-sm font-medium text-gray-400">No active members yet</p>
                        <p className="text-xs text-gray-300 mt-1">Invite sellers above to build your team</p>
                    </motion.div>
                ) : (
                    <div className="bg-white border border-gray-100/80 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
                            ...
                            {filteredActive.map((m: any, i: number) => (
                                <motion.div
                                    ...
                                    className={`flex items-center justify-between px-5 py-3.5 hover:bg-gray-50/60 transition-colors group ${
                                        i < filteredActive.length - 1 ? 'border-b border-gray-50' : ''
                                    }`}
                                >
                                    <div className="flex items-center gap-3.5">
                                        <MemberAvatar name={m.Seller?.name || m.Seller?.email || '?'} />
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-gray-200 opacity-0 group-hover:opacity-100 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all active:scale-[0.95]"
                                        title="Remove member"
                                    >
                                        {actionLoading === m.id ? (
                                            <Loader2 className="w-4 h-4 animate-spin text-gray-300" />
                        ...
                        {searchQuery && filteredActive.length === 0 && (
                            <div className="px-5 py-10 text-center">
                                <p className="text-sm text-gray-400">No members match &ldquo;{searchQuery}&rdquo;</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            {removedMembers.length > 0 && (
                ...
                >
                    <button
                        ...
                        className="flex items-center gap-2 text-xs text-gray-400 hover:text-gray-500 transition-colors mb-3"
                    >
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                {m.Seller?.email && m.Seller?.name && (
                                                    <p className="text-xs text-gray-300">{m.Seller?.email}</p>
                                                )}
                                            </div>
                                        </div>
                                    ))}
